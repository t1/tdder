 ...
 * Queries are sent over the Neo4j HTTP transactional API (port 7474) using plain fetch — no npm dependency needed.
 */
...
import {existsSync, readdirSync, readFileSync, statSync, watch as fsWatch, writeFileSync} from "node:fs";
...
function sentinelMtime(cwd: string): number {
    try {
        return statSync(sentinelPath(cwd)).mtimeMs;
    } catch {
        return 0;
    }
}
...
export default function jqaExtension(pi: ExtensionAPI) {
     ...
     */
    async function performScan(cwd: string, ctx: UiCtx, signal?: AbortSignal): Promise<{
        ok: boolean;
        message: string
    }> {
    ...
    async function applyConcept(concept: Rule, httpPort: number): Promise<string> {
        if (!concept.cypher) return `${concept.id}: no Cypher`;
        const count = await runWriteCypher(concept.cypher, httpPort);
        return `${concept.id}: ${count} affected`;
    }

    async function runConstraint(constraint: Rule, httpPort: number): Promise<{ table: string; violations: number }> {
        if (!constraint.cypher) return {table: "(no Cypher defined)", violations: 0};
        const {columns, rows} = await runCypher(constraint.cypher, httpPort);
    ...
    async function showRulesExplorer(cwd: string, ctx: any): Promise<void> {
        ...
        let ruleset: RuleSet;
        try {
            ruleset = loadAllRules(extensionCwd);
        } finally {
        ...
        const buildItems = (): SelectItem[] => {
            ...
            if (groups.size > 0) {
                items.push({
                    value: "__header_groups__",
                    label: `── Groups (${groups.size}) ──`,
                    description: "named sets of rules"
                });
            ...
            items.push({
                value: "__header_constraints__",
                label: `── Constraints (${constraints.size}) ──`,
                description: "run to find violations"
            });
            for (const c of constraints.values()) {
                ...
                items.push({
                    value: `constraint:${c.id}`,
                    label: c.id,
                    description: c.description.slice(0, 80) + depsNote
                });
            }
            ...
            items.push({
                value: "__toggle_concepts__",
                label: conceptToggleLabel,
                description: "graph-enriching write queries"
            });
        ...
        await ctx.ui.custom<void>((tui: any, theme: any, _kb: any, done: (v: void) => void) => {
            ...
            const styledTheme: SelectListTheme = {
            ...
            selectList.onSelect = async (item: SelectItem) => {
                ...
                if (val === "__toggle_concepts__") {
                    conceptsExpanded = !conceptsExpanded;
                    rebuild();
                    return;
                }
                ...
                if (val.startsWith("concept:")) {
                    ...
                    try {
                        const boltPort = await ensureServer(extensionCwd, ctx);
                ...
                if (val.startsWith("constraint:")) {
                    ...
                    try {
                        ...
                        pi.sendMessage({
                            ...
                            content: table,
                            display: true,
                            details: {header, cypher: constraint.cypher}
                        });
                    } catch (e: any) {
    ...
    async function ensureFresh(cwd: string, ctx: UiCtx, signal?: AbortSignal): Promise<number> {
        if (storeIsStale) {
            const {ok, message} = await performScan(extensionCwd, ctx, signal);
            if (!ok) throw new Error(`Auto-scan failed: ${message}`);
        }
        return ensureServer(extensionCwd, ctx);
    }
    ...
    pi.registerTool({
        ...
        renderCall(args, theme) {
            const text = theme.fg("toolTitle", theme.bold("jqa_query")) + "  " + theme.fg("dim", args.cypher);
        ...
        async execute(_id, params, signal) {
            ...
            try {
                const port = await ensureFresh(extensionCwd, ctx, signal ?? undefined);
    ...
    pi.registerTool({
        ...
        async execute(_id, _params, signal) {
            ...
            const rulesDir = join(extensionCwd, "jqassistant", "rules");
            if (!existsSync(rulesDir)) {
                return {
                    content: [{
                        type: "text",
                        text: "No rules directory found at jqassistant/rules/. Use jqa_add_rule_to_config to add rules, or jqa_list_rules to browse available rules."
                    }],
                    isError: true,
                };
            }
            ...
            try {
                if (storeIsStale) {
                    const {ok, message} = await performScan(extensionCwd, ctx, signal ?? undefined);
                    if (!ok) return {content: [{type: "text", text: `Auto-scan failed: ${message}`}], isError: true};
                }
                const {stdout, stderr, code} = await runJqa("analyze", [], extensionCwd, signal ?? undefined);
                const out = stripLogNoise(stdout + "\n" + stderr);
                return {
    ...
    pi.registerTool({
        ...
        renderCall(args, theme) {
            const filterLabel = args.filter ?? "(all)";
            const text = theme.fg("toolTitle", theme.bold("jqa_list_rules")) + "  " + theme.fg("dim", `filter: ${filterLabel}`);
            return new Text(text, 0, 0);
        },
        ...
        async execute(_id, params) {
            const ruleset = loadAllRules(extensionCwd);
            const {concepts, constraints, groups} = ruleset;
        ...
        async execute(_id, params, signal) {
            ...
            if (!concept) {
                return {
                    content: [{
                        type: "text",
                        text: `Concept '${params.conceptId}' not found. Use jqa_list_rules to browse available concepts.`
                    }],
                    isError: true,
                };
            }
            ...
            const ctx = makeToolCtx();
            try {
                const boltPort = await ensureFresh(extensionCwd, ctx, signal ?? undefined);
    ...
    pi.registerTool({
        ...
        renderCall(args, theme) {
            const text = theme.fg("toolTitle", theme.bold("jqa_run_constraint")) + "  " + theme.fg("accent", args.constraintId);
            return new Text(text, 0, 0);
        },
        ...
        async execute(_id, params, signal) {
            ...
            if (!constraint) {
                return {
                    content: [{
                        type: "text",
                        text: `Constraint '${params.constraintId}' not found. Use jqa_list_rules to browse available constraints.`
                    }],
                    isError: true,
                };
            }
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            try {
                const message = addRuleToConfig(extensionCwd, params.ruleId, params.kind);
    ...
    pi.registerCommand("jqa", {
        ...
        getArgumentCompletions: (prefix) => {
            const subs = [
                ...
                {value: "query", label: "query   – Run a Cypher query"},
        ...
        handler: async (args, ctx) => {
            ...
            switch (sub) {
                ...
                case "query": {
                    ...
                    try {
                        ...
                        pi.sendMessage({
                            ...
                            content: table,
                            display: true,
                            details: {cypher, rows: rows.length}
                        });
                    } catch (err) {
                ...
                case "explore": {
                    await showRulesExplorer(extensionCwd, ctx);
                    break;
                }
                ...
                case "start": {
                    if (server.process && server.ready) {
                        ctx.ui.notify(`jQAssistant server is already running (http::${server.httpPort}, browser→ http://localhost:${server.httpPort})`, "info");
                    } else {
                        try {
