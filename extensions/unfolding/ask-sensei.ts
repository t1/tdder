...
export async function askSenseiViaUi(
  ...
  const proxiedRootMode = (ctx.ui as any)?.__unfoldingRootMode;
  if (ctx.mode === "tui" && (!proxiedChildUi || proxiedRootMode === "tui")) return askSenseiViaQuestionnaire(prompt, options, ctx);
