...
Then read the Feature description from your `[API]` task. Then load `docs/ats/INDEX.md`
...
hasn't specified listing owners), **STOP and message the PO** rather than
...
role names, do NOT normalize them yourself — STOP and ask the PO which term
is correct.
...
1. Mark your `[API]` task as complete
2. Message the PO with:
    - References to new and existing resources (by file path)
    - The interaction flow for the Feature (sequence of API calls)
    - Any convention updates
    - A **change summary** listing file changes:
...
- Do NOT silently accept a Feature spec that feels wrong from an API
  consumer's perspective — challenge it by STOPping with questions
